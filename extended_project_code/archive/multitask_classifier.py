...
from src.evaluation import model_eval_sst, model_eval_multitask, model_eval_test_multitask
...
class SentimentClassifier(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.n_class = N_SENTIMENT_CLASSES
...
class MultitaskBERT(nn.Module):
    ...
    def predict_paraphrase(
        ...
        """
        embed1 = self.forward(input_ids_1, attention_mask_1)
        embed2 = self.forward(input_ids_2, attention_mask_2)
        return self.paraphrase_classifier(embed1, embed2)
    ...
    def predict_similarity(
        ...
        embed1 = self.forward(input_ids_1, attention_mask_1)
        embed2 = self.forward(input_ids_2, attention_mask_2)
        return self.similarity_classifier(embed1, embed2)
...
def get_args():
    ...
    parser.add_argument(
        "--fine-tune-mode",
        type=str,
        help="last-linear-layer: the BERT parameters are frozen and the task specific "
             "head parameters are updated; full-model: "
             "BERT parameters are updated as well",
