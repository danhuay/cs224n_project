...
def load_yaml_to_simplenamespace(filename):
    data = load_yaml_to_dict(filename)
...
def dict_to_simplenamespace(d):
    """
    Converts dictionary to simple name spaces so it can be called like obj.arg
    :param d: dictionary
    :return: simple name space
    """
...
def parallel_processing(func, args_list):
    num_workers = floor(mp.cpu_count() * 0.9)

    # Initialize Pool with the number of available CPUs
    with mp.Pool(processes=num_workers) as pool:
        # Map parallel_run over args_list
        res_holder = pool.starmap(func, args_list)

    filtered_res_holder = [ele for ele in res_holder if ele is not None]

    return filtered_res_holder
