...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        add_matrix = input + dropout(sublayer)
        layer_norm_output = ln_layer(add_matrix)
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        """
        # Multi-head attention (before linearly transformed by W^O).
        multi_head = self.self_attention(hidden_states, attention_mask)
        
        # Add-norm for multi-head attention.
        add_norm_mh = self.add_norm(hidden_states, multi_head, self.attention_dense,
                                    self.attention_dropout, self.attention_layer_norm)
        # Feed forward.
        self_attn = self.interm_af(self.interm_dense(add_norm_mh))
        
        # Add-norm for feed forward.
        hidden_states_ff = self.add_norm(add_norm_mh, self_attn, self.out_dense,
...
class BertModel(BertPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        inputs_embeds = self.word_embedding(input_ids)  # size: (batch_size, seq_len, embed_size=768)
        
        # Use pos_ids to get position embedding from self.pos_embedding into pos_embeds.
        pos_ids = self.position_ids[:, :seq_length]
        pos_embeds = self.pos_embedding(pos_ids) # size: (1, seq_len, embed_size=768)
